...
@implementation WKChineseSortSetting

+(WKChineseSortSetting*)share{
    static WKChineseSortSetting * singleton = nil ;
    if (singleton == nil) {
        singleton = [[WKChineseSortSetting alloc] init];
...
@implementation WKChineseSortModel
@end
...
- (NSComparisonResult)mySort:(NSString *)str {
    NSString*s = [WKChineseSortSetting share].specialCharSectionTitle;
    BOOL b = [WKChineseSortSetting share].specialCharPositionIsFront;
...
+(void)sortAndGroup:(NSArray*)objectArray
    ...
    if (key == nil) {
        if (![obj isKindOfClass:NSString.class]) {
            [WKChineseSort logMsg:@"数组内元素不是字符串类型,如果是对象类型，请传key"];
    ...
    if (!containKey) {
        [WKChineseSort logMsg:@"数组内元素未包含指定属性"];
        finish(NO,nil,nil,nil);
        return;
    }
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_LOW, 0), ^{
        ...
        [WKChineseSort logMsg:[NSString stringWithFormat:@"排序用时：\t %f s", (state2-state1)]];
        ...
        for (WKChineseSortModel* object in sortModelArray) {
        ...
        [WKChineseSort logMsg:[NSString stringWithFormat:@"WKChineseSort 排序总计用时：\t %f s", (state3-start)]];
...
//将对象 转为 WKChineseSortModel
+(WKChineseSortModel*)getModelWithObj:(id)obj key:(NSString*)key{
    WKChineseSortModel *model = [[WKChineseSortModel alloc]init];
    model.object = obj;
    if (!key) {
        model.string = obj;
    }else{
    ...
    if(model.string == nil || model.string.length == 0){
        model.string = WKChineseSortSetting.share.specialCharSectionTitle;
        model.pinYin = @"#";
    }else{
        //过滤 ignoreModelWithPrefix
        NSString *prefix = [model.string substringToIndex:1];
        if (![WKChineseSortSetting.share.ignoreModelWithPrefix containsString:prefix]) {
            //是否将字母与汉字拼音一起排序？？？？ （暂不考虑）
            //获取拼音首字母
...
+(void)logMsg:(NSString*)msg{
    if (WKChineseSortSetting.share.logEable == YES) {
        NSLog(@"------------------");
        NSLog(@"%@", msg);
        NSLog(@"------------------");
    }
...
+ (NSString *)getFirstLetter:(NSString *)chinese{
    ...
    //吧多音字先替换
    [WKChineseSortSetting.share.polyphoneMapping enumerateKeysAndObjectsUsingBlock:^(id  _Nonnull key, id  _Nonnull obj, BOOL * _Nonnull stop) {
        newChinese = [newChinese stringByReplacingOccurrencesOfString:key withString:obj];
    }];

    NSMutableString *result = [NSMutableString string];

    if (WKChineseSortSetting.share.sortMode == 1) {
...
char pinyinFirstLetter(unsigned short hanzi){
    //大写字母
    if (hanzi >= 65 && hanzi <= 90){
        return (char)hanzi;
    }
    //小写字母
    if (hanzi >= 97 && hanzi <= 122){
        return (char)hanzi;
    }
    //汉字
