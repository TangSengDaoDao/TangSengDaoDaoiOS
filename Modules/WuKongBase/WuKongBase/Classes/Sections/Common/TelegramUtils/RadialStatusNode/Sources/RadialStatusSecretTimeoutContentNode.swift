...
final class RadialStatusSecretTimeoutContentNode: RadialStatusContentNode {
    ...
    override func willEnterHierarchy() {
        ...
        print("willEnterHierarchy----->")
    }
    
    override func didExitHierarchy() {
        super.didExitHierarchy()
        self.displayLink?.isPaused = true
        print("didExitHierarchy----->")
    }
    ...
    private func displayLinkEvent() {
        ...
        print("self.progress--->",self.progress,CFAbsoluteTimeGetCurrent(),self.beginTime);
        if(self.progress >= 1.0 && !self.complete) {
            self.complete = true
