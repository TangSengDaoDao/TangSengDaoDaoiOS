...
@objc public final class RadialStatusNode: ASControlNode {
    ...
    @objc public var completeCallback: ()->Void = { }
    
   @objc public init(backgroundNodeColor: UIColor, enableBlur: Bool = false) {
    ...
    @objc public func transitionToState(icon:UIImage,beginTime:Double,timeout:Double,animated: Bool = true, synchronous: Bool = false,sparks:Bool = true, completion: @escaping () -> Void = {}) {
        self.transitionToState(.secretTimeout(color: .white, icon: icon, beginTime: beginTime, timeout: timeout, sparks: sparks), animated: animated, synchronous: synchronous, completion: completion);
    }
    @objc public func transitionToState(icon:UIImage,beginTime:Double,timeout:Double,animated: Bool = true, synchronous: Bool = false,sparks:Bool = true, finished: @escaping () -> Void = {}) {
        self.transitionToState(.secretTimeout(color: .white, icon: icon, beginTime: beginTime, timeout: timeout, sparks: sparks), animated: animated, synchronous: synchronous, finished: finished);
    }
    
    public func transitionToState(_ state: RadialStatusNodeState, animated: Bool = true, synchronous: Bool = false, completion: @escaping () -> Void = {}, finished: @escaping () -> Void = {}) {
        ...
        if self.state != state {
            ...
            if contentNode !== self.contentNode {
                if(contentNode is RadialStatusSecretTimeoutContentNode) {
                    (contentNode as! RadialStatusSecretTimeoutContentNode).completeCallback = finished
                }
   ...
   @objc public func animatePaused() {
        if self.contentNode is RadialStatusSecretTimeoutContentNode {
            (self.contentNode as! RadialStatusSecretTimeoutContentNode).animatePaused();
        }
    }
    @objc public func animateStart() {
        if self.contentNode is RadialStatusSecretTimeoutContentNode {
            (self.contentNode as! RadialStatusSecretTimeoutContentNode).animateStart();
        }
    }
}
