...
final class ContextControllerExtractedPresentationNode: ASDisplayNode, ContextControllerPresentationNode, UIScrollViewDelegate {
    ...
    private final class ContentNode: ASDisplayNode {
        ...
        var containingNode: ContextExtractedContentContainingNode
        ...
        func resetContainingNode(containingNode: ContextExtractedContentContainingNode) {
            // 将旧cell还原到tableview上
            self.containingNode.contentNode.removeFromSupernode()
            self.containingNode.addSubnode(self.containingNode.contentNode)
            // 偏移作用到新的contentNode上
            self.containingNode = containingNode
            self.offsetContainerNode.addSubnode(self.containingNode.contentNode)
        }
    ...
    public var needUpdateContentNode: Bool = false
...
//        self.dismissTapNode.backgroundColor = .black
...
//        self.scrollNode.backgroundColor = .blue
    ...
    func update(
        ...
        if let current = self.contentNode {
            ...
            case let .extracted(source):
                ...
                // 重置当前消息新的cell（当tableView reloadData的时候原来持有的cell会变成其他消息的）
                current.resetContainingNode(containingNode: takeInfo.contentContainingNode)
               
            case .reference(_): break
            ...
            case .reference:
                contentNode = nil
            case let .extracted(source):
                NSLog("update---extracted....");
                guard let takeInfo = source.takeView() else {
                    return
                }
                NSLog("update---extracted22....");
