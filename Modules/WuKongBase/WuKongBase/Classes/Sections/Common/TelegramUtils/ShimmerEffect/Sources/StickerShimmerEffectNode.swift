...
@objc public class StickerShimmerEffectNode: ASDisplayNode {
    ...
    @objc public func updateAbsoluteRect(_ rect: CGRect, within containerSize: CGSize) {
        self.effectNode.updateAbsoluteRect(rect, within: containerSize)
    }
    
    @objc public func update(backgroundColor: UIColor?, foregroundColor: UIColor, shimmeringColor: UIColor, data: Data?, size: CGSize, imageSize: CGSize = CGSize(width: 512.0, height: 512.0), isDecode: Bool=true){
        if data == nil {
            return
        }
        ...
        let image = generateImage(size, rotatedContext: { size, context in
            ...
            if let data = data {
                var path: String
                if(isDecode) {
                     path = decodeStickerThumbnailData(data)
                }else {
