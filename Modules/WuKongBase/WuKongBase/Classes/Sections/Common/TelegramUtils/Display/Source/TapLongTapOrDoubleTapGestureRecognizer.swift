            ...
            if self.tapCount == 2 && self.touchCount == 1 {
                ...
                switch tapAction {
                    ...
                    case .waitForSingleTap, .waitForDoubleTap:
                        self.timer?.invalidate()
                    let timer = Foundation.Timer(timeInterval: 0.3, target: TapLongTapOrDoubleTapGestureRecognizerTimerTarget(target: self), selector: #selector(TapLongTapOrDoubleTapGestureRecognizerTimerTarget.longTapEvent), userInfo: nil, repeats: false)
                        self.timer = timer
                        RunLoop.main.add(timer, forMode: .common)
                    case let .waitForHold(timeout, _):
                        self.hapticFeedback = HapticFeedback()
                        self.hapticFeedback?.prepareTap()
                    let timer = Foundation.Timer(timeInterval: timeout, target: TapLongTapOrDoubleTapGestureRecognizerTimerTarget(target: self), selector: #selector(TapLongTapOrDoubleTapGestureRecognizerTimerTarget.holdEvent), userInfo: nil, repeats: false)
                        self.timer = timer
                        RunLoop.main.add(timer, forMode: .common)
                    case .fail:
                        self.state = .failed
    ...
    override public func touchesEnded(_ touches: Set<UITouch>, with event: UIEvent) {
        ...
        if self.tapCount == 1 {
            ...
            switch tapAction {
                ...
                case .waitForDoubleTap:
                    self.state = .began
                let timer = Foundation.Timer(timeInterval: 0.16, target: TapLongTapOrDoubleTapGestureRecognizerTimerTarget(target: self), selector: #selector(TapLongTapOrDoubleTapGestureRecognizerTimerTarget.tapEvent), userInfo: nil, repeats: false)
                    self.timer = timer
                    RunLoop.main.add(timer, forMode: .common)
