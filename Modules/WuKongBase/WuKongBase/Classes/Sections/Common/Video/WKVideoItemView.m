...
- (void)updateLayoutWithExpectOrientation:(UIDeviceOrientation)orientation containerSize:(CGSize)containerSize {
    ...
    self.topBar.frame = CGRectMake(padding.left, padding.top, width, [WKVideoTopBar defaultHeight]);
    self.actionBar.frame = CGRectMake(padding.left, height - [WKVideoTopBar defaultHeight] - padding.bottom - 10, width, [WKVideoActionBar defaultHeight]);
...
//        _resourceLoaderManager = resourceLoaderManager;
        _playerItem = [AVPlayerItem playerItemWithURL:((AVURLAsset*)self.asset).URL];
...
- (void)playerItemStatusChanged {
    ...
    switch (_playerItem.status) {
        ...
        case AVPlayerItemStatusUnknown: {
            _playFailed = YES;
            NSLog(@"_playerItem.error--->%@",_playerItem.error);
...
- (BOOL)resourceLoader:(AVAssetResourceLoader *)resourceLoader shouldWaitForResponseToAuthenticationChallenge:(NSURLAuthenticationChallenge *)challenge
{
    ...
    [[self customSecurityPolicy] evaluateServerTrust:serverTrust forDomain:nil];
...
- (WKVideoTopBar *)topBar {
    if (!_topBar) {
        _topBar = [WKVideoTopBar new];
...
- (WKVideoActionBar *)actionBar {
    if (!_actionBar) {
        _actionBar = [WKVideoActionBar new];
...
- (UIButton *)playButton {
    if (!_playButton) {
        ...
        [_playButton setImage:[self getImageWithName:@"Conversation/VideoBrowser/ybib_bigPlay"] forState:UIControlStateNormal];
