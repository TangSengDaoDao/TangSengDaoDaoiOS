...
// Public Constants
NSString *const WKDBMigrationManagerErrorDomain = @"com.layer.WKDBMigrationManager.errors";
NSString *const WKDBMigrationManagerProgressVersionUserInfoKey = @"version";
NSString *const WKDBMigrationManagerProgressMigrationUserInfoKey = @"migration";

// Private Constants
static NSString *const WKDBMigrationFilenameRegexString = @"^(\\d+)_?((?<=_)[\\w\\s-]+)?(?<!_)\\.sql$";
...
BOOL FMDBIsMigrationAtPath(NSString *path)
{
    ...
    dispatch_once(&onceToken, ^{
        migrationRegex = [NSRegularExpression regularExpressionWithPattern:WKDBMigrationFilenameRegexString options:0 error:nil];
    });
...
@interface WKDBMigrationManager ()
...
@implementation WKDBMigrationManager
...
- (void)addMigrations:(NSArray *)migrations
{
    ...
    for (id<NSObject> migration in migrations) {
        if (![migration conformsToProtocol:@protocol(WKDBMigrating)]) {
            @throw [NSException exceptionWithName:NSInvalidArgumentException reason:@"Failed to add migrations because an object in `migrations` array doesn't conform to the `WKDBMigrating` protocol." userInfo:nil];
        }
    }
...
- (NSArray *)migrations
{
  ...
  //  if (_migrations) return _migrations;
    ...
    NSArray *migrationPaths = [self.migrationsBundle pathsForResourcesOfType:@"sql" inDirectory:@""];
    NSRegularExpression *migrationRegex = [NSRegularExpression regularExpressionWithPattern:WKDBMigrationFilenameRegexString options:0 error:nil];
    ...
    for (NSString *path in migrationPaths) {
        ...
        if ([migrationRegex rangeOfFirstMatchInString:filename options:0 range:NSMakeRange(0, [filename length])].location != NSNotFound) {
            WKDBFileMigration *migration = [WKDBFileMigration migrationWithPath:path];
            [migrations addObject:migration];
        }
    }
    
    // Find all classes implementing WKDBMigrating
    if (self.dynamicMigrationsEnabled) {
        NSArray *conformingClasses = FMDBClassesConformingToProtocol(@protocol(WKDBMigrating));
        for (Class migrationClass in conformingClasses) {
            if ([migrationClass isSubclassOfClass:[WKDBFileMigration class]]) continue;
            id<WKDBMigrating> migration = [migrationClass new];
            [migrations addObject:migration];
        }
    }
...
{
    for (id<WKDBMigrating>migration in [self migrations]) {
...
- (BOOL)migrateDatabase:(NSBundle*)migrationsBundle error:(NSError **)error{
   ...
   return  [self migrateDatabaseToVersion:UINT64_MAX bundle:migrationsBundle  progress:nil error:error];
}
...
- (BOOL)migrateDatabaseToVersion:(uint64_t)version bundle:(NSBundle*)migrationsBundle progress:(void (^)(NSProgress *progress))progressBlock error:(NSError **)error
{
     ...
     if(!_migrationsBundle) [NSException exceptionWithName:NSInternalInconsistencyException reason:@"migrationsBundle is nil" userInfo:nil];
    ...
    for (NSNumber *migrationVersionNumber in pendingVersions) {
        ...
        id<WKDBMigrating> migration = [self migrationForVersion:migrationVersion];
        ...
        if (progressBlock) {
            [progress setUserInfoObject:@(migrationVersion) forKey:WKDBMigrationManagerProgressVersionUserInfoKey];
            [progress setUserInfoObject:migration forKey:WKDBMigrationManagerProgressMigrationUserInfoKey];
            progressBlock(progress);
            if (progress.cancelled) {
                success = NO;
                
                NSDictionary *userInfo = @{ NSLocalizedDescriptionKey: @"Migration was halted due to cancellation." };
                if (error) *error = [NSError errorWithDomain:WKDBMigrationManagerErrorDomain code:WKDBMigrationManagerErrorMigrationCancelled userInfo:userInfo];
                [self.database rollback];
                break;
            }
        }
...
static BOOL WKDBMigrationScanMetadataFromPath(NSString *path, uint64_t *version, NSString **name)
{
    NSError *error = nil;
    NSRegularExpression *regex = [NSRegularExpression regularExpressionWithPattern:WKDBMigrationFilenameRegexString options:0 error:&error];
    if (!regex) {
        NSLog(@"[WKDBMigration] Failed constructing regex: %@", error);
        return NO;
    }
...
@interface WKDBFileMigration ()
@property (nonatomic, readwrite) NSString *name;
@property (nonatomic, readwrite) uint64_t version;
@end

@implementation WKDBFileMigration
...
- (id)initWithPath:(NSString *)path
{
    NSString *name;
    uint64_t version;
    if (!WKDBMigrationScanMetadataFromPath(path, &version, &name)) return nil;
